...
export async function handleWebSub(ctx: Context, req: Request): Promise<Response | undefined> {
	...
	queueMicrotask(async () => {
		...
		await db.youtubeChannel.update({ where: { webhookId: id }, data: { webhookExpiresAt: null, webhookSecret: null } });
		const chData = await getChannelData(`https://youtube.com/channel/${ytCh.id}`);
...
async function videoCallback(ctx: Context, ch: YoutubeChannel, body: Buffer) {
	...
	if (videoRecord.liveNotifiedAt || videoRecord.upcomingNotifiedAt || videoData.schedule === videoRecord.scheduledAt) {
		lock.delete(videoId);
		return;
	}
