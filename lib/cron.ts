		...
		const chs = await db.youtubeChannel.findMany({
			where: {
				AND: [
					...
					{ OR: [{ webhookExpiresAt: { lte: new Date(Date.now() - 24 * 3600000) } }, { webhookExpiresAt: null }] }
				]
			},
			select: {
				id: true,
	...
	cron.schedule('*/5 * * * *', async () => {
		...
		for (const videoRecord of records) {
			...
			const videoData = await getVideoData(videoRecord.id, false).catch((err) => {
				...
				console.warn('[cron]', `Failed to fetch video data for https://www.youtube.com/watch?v=${videoRecord.id}`, {
					error: err
				});
				return null;
			});
